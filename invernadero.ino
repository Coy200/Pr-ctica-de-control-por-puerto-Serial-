...
// Pin de control del foco
const int ssrPin = 26;

// Número de lecturas para promediar
const int numLecturas = 10;

float desiredTemp = 25.0; // Temperatura deseada por el usuario
...
void setup() {
  ...
  pinMode(ssrPin, OUTPUT); // Configura el pin del foco como salida
...
void loop() {
  ...
  // Controlar el foco incandescente según la temperatura promedio
  if (promedio < desiredTemp) {
    digitalWrite(ssrPin, HIGH); // Encender el foco
  } else {
    digitalWrite(ssrPin, LOW); // Apagar el foco
  }
